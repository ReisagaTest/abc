...
local function verifyToken(data, userKey, secretKey)
    if not (data and data.nonce and data.timestamp and data.signature and data.userKey and userKey) then
        return false
    end
    if data.userKey ~= userKey then
        return false
    end
    local tokenData = data.nonce .. ":" .. data.timestamp .. ":" .. userKey
    local expectedSignature = HttpService:UrlEncode(tokenData .. secretKey)
    if expectedSignature ~= data.signature then
        return false
    end
    return math.abs(os.time() - tonumber(data.timestamp)) <= 300
end

local function hookAntiCheat()
    setthreadidentity(2)
    for _, v in ipairs(getgc(true)) do
        if typeof(v) == "table" then
            ...
            if rawget(v, "Variables") and rawget(v, "Process") and typeof(killFunc) == "function" then
...
local function antiKick()
    getgenv().ED_AntiKick = { Enabled = true, SendNotifications = true }
    local mt = getrawmetatable(LocalPlayer)
    local oldIndex, oldNamecall = mt.__index, mt.__namecall
    setreadonly(mt, false)

    mt.__index = newcclosure(function(self, index)
        if type(index) == "string" and index:lower() == "kick" and getgenv().ED_AntiKick.Enabled then
            if getgenv().ED_AntiKick.SendNotifications then
                StarterGui:SetCore("SendNotification", {
                    Title = "Anti-Kick",
                    Text = "Intercepted kick attempt.",
                    Icon = "rbxassetid://",
                    Duration = 2
                })
            end
    ...
    mt.__namecall = newcclosure(function(self, index, ...)
        if getnamecallmethod():lower() == "kick" and getgenv().ED_AntiKick.Enabled then
            if getgenv().ED_AntiKick.SendNotifications then
                StarterGui:SetCore("SendNotification", {
                    Title = "Anti-Kick",
                    Text = "Intercepted kick attempt.",
...
local function protectIP()
    getgenv().IpProtection = true
    local spoofedIP = "0.0.0.0"
    local trackers = {
        "https://v4.ident.me",
        "https://api.ipify.org",
    ...
    hookfunction(game.HttpGet, function(self, url)
        if getgenv().IpProtection and type(url) == "string" and table.find(trackers, url) then
            return spoofedIP
        end
        return game.HttpGet(self, url)
    end)

    hookfunction((syn and syn.request or request), function(params)
        if getgenv().IpProtection and type(params) == "table" and params.Url and table.find(trackers, params.Url) then
            return { StatusCode = 200, Body = spoofedIP }
        end
        return (syn and syn.request or request)(params)
    end)
end

local function createSignalManager()
    local manager = { Signals = {} }

    function manager:add(name)
        self.Signals[name] = createSignal(name)
    end
    ...
    function manager:connect(name, callback)
        local signal = self.Signals[name]
        if signal then return signal:connect(callback) end
    end

    function manager:wait(name, timeout)
        local signal = self.Signals[name]
        if signal then return signal:wait(timeout) end
    end
...
local function createSignal(name)
    local signal = { Name = name, Connections = {} }
...
local function createConnection(signal, callback)
    local connection = { Callback = callback, State = true, Signal = signal }

    function connection:disconnect()
        for i, conn in ipairs(signal.Connections) do
            if conn == self then
...
local function optimizeUI(guiObject, blurEnabled, lightingEnabled, detailLevel)
    local targetFPS = 60
    local maxFrameTime = 1 / targetFPS
    local lastFrameTime = os.clock()

    local function adjustBlur(enabled, quality)
        local blurEffect = guiObject:FindFirstChildOfClass("BlurEffect") or Instance.new("BlurEffect")
        blurEffect.Name = "UIBlur"
        blurEffect.Parent = enabled and guiObject or nil
        blurEffect.Size = enabled and (quality == "low" and 2 or quality == "medium" and 8 or 16) or 0
        if not enabled and blurEffect.Parent then blurEffect:Destroy() end
    end

    local function adjustLighting(enabled, quality)
        local canvasGroup = guiObject:FindFirstChildOfClass("CanvasGroup") or Instance.new("CanvasGroup")
        canvasGroup.Name = "UILighting"
        canvasGroup.Parent = enabled and guiObject or nil
        canvasGroup.GroupTransparency = enabled and (quality == "low" and 0.2 or quality == "medium" and 0.4 or 0.6) or 1
        if not enabled and canvasGroup.Parent then canvasGroup:Destroy() end
    end

    RunService.RenderStepped:Connect(function()
        local currentTime = os.clock()
        local elapsedTime = currentTime - lastFrameTime

        if elapsedTime > maxFrameTime then
            if blurEnabled and detailLevel ~= "low" then
                detailLevel = detailLevel == "high" and "medium" or "low"
            elseif lightingEnabled and detailLevel ~= "low" then
                detailLevel = detailLevel == "high" and "medium" or "low"
            elseif detailLevel == "high" then
                detailLevel = "medium"
        ...
        adjustBlur(blurEnabled, detailLevel)
        adjustLighting(lightingEnabled, detailLevel)
        lastFrameTime = currentTime
    end)
end

local function manageCoreGui()
    local metatable = setmetatable({}, { __mode = "kv" })
    metatable[1] = game:GetService("CoreGui")
    metatable[2] = {}

    RunService.Heartbeat:Connect(function()
        if metatable[1] and not metatable[2] then
            LocalPlayer:Kick()
        end
    end)
end

local function setupD3D9Hooks()
    local d3d9Hooks = {
        D3DPERF_BeginEvent = function() end,
        D3DPERF_EndEvent = function() end,
        D3DPERF_GetStatus = function() return 0 end,
        D3DPERF_QueryRepeatFrame = function() return false end,
        D3DPERF_SetMarker = function() end,
        D3DPERF_SetOptions = function() end,
        D3DPERF_SetRegion = function() end,
        DebugSetLevel = function() end,
        DebugSetMute = function() end,
        Direct3D9EnableMaximizedWindowedModeShim = function() return true end,
        Direct3DCreate9 = function() return {} end,
        Direct3DCreate9Ex = function() return {} end,
        Direct3DShaderValidatorCreate9 = function() return {} end,
        PSGPError = function() end,
        PSGPSampleTexture = function() end
    }
    for hookName, func in pairs(d3d9Hooks) do
        rawset(_G, "Fake" .. hookName, func)
    end
end
...
task.spawn(function()
    ...
    manageCoreGui()
    setupD3D9Hooks()
    optimizeUI(ScreenGui, true, true, "high")
end)
...
local RunService = game:GetService("RunService")
local dodgeParts = {
    "ExplodeFloor", "Spiral", "eff", "CharacterHit", "PersistentParts",
    "SkinnedCylinders", "EagleTAPEffect", "ChargeUp", "ExplosionStartimpact",
    "ExplosionStartimpact2", "SharkSplash", "GroundDroplets", "GroundBoulder",
    "Ring", "Fist", "MochiSwirl"
}
local function applySpin(rootPart)
    local spinCFrame = CFrame.Angles(0, math.rad(getgenv().SpinSpeed * tick()), 0)
    rootPart.CFrame = rootPart.CFrame * spinCFrame
end

local function applyRandomPitch(rootPart)
    local randomPitch = CFrame.Angles(math.rad(math.random(-45, 45)), math.rad(math.random(-45, 45)), 0)
    rootPart.CFrame = rootPart.CFrame * randomPitch
end

local function isThreatActive(source)
    if not source or not source:FindFirstChild("HumanoidRootPart") then return false end
    for _, descendant in ipairs(source:GetDescendants()) do
        if descendant:IsA("ParticleEmitter") and descendant.Enabled then return true end
    end
    local humanoid = source:FindFirstChild("Humanoid")
    if humanoid then
        local animator = humanoid:FindFirstChild("Animator")
        if animator then
            for _, anim in ipairs(animator:GetPlayingAnimationTracks()) do
                if anim.Name:lower():find("attack") then return true end
            end
        end
    end
    return false
...
local function getRandomDodgeCFrame(rootPos)
    local randomOffset = Vector3.new(math.random(-1, 1), 0, math.random(-1, 1)).Unit * 25
    return CFrame.new(rootPos + randomOffset)
end

local function dodgeWorldOrigin(rootPart)
    local worldOrigin = game.Workspace:FindFirstChild("_WorldOrigin")
    if not worldOrigin then return false end
    
    for _, part in ipairs(worldOrigin:GetDescendants()) do
        if table.find(dodgeParts, part.Name) then
            local distance = (rootPart.Position - part.Position).Magnitude
            if distance < 25 then
                local targetCFrame = getRandomDodgeCFrame(rootPart.Position)
                topos(rootPart, targetCFrame)
                applyRandomPitch(rootPart)
                return true
            end
        end
    end
...
local function dodgeSkills(rootPart, source, sourceType)
    if isThreatActive(source) then
        local skill = source:FindFirstChild("Skill")
        local root = source:FindFirstChild("HumanoidRootPart")
        if skill and root then
            local distance = (rootPart.Position - skill.Position).Magnitude
            if distance < 25 then
                local targetCFrame = getRandomDodgeCFrame(rootPart.Position)
                topos(rootPart, targetCFrame)
                applyRandomPitch(rootPart)
...
local function autoDodge()
    ...
    applySpin(rootPart)
    if dodgeWorldOrigin(rootPart) then return end
    for _, enemy in pairs(game.Workspace.Enemies:GetChildren()) do
        if dodgeSkills(rootPart, enemy, "Enemy") then return end
    end
    for _, player in pairs(game.Players:GetChildren()) do
        local char = player.Character
        if char and char ~= character then
            if dodgeSkills(rootPart, char, "Player") then return end
        end
    end
end
...
Toggle = Stack:AddToggle("Toggle", { Title = "Auto Fully Tyrant", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().AutoFullyTyrant = Value
    StopTween(getgenv().AutoFullyTyrant)
end)
    ...
    for _, model in ipairs(islandModel:GetChildren()) do
        ...
        if arena then
            for _, v in ipairs(arena:GetChildren()) do
                if v.Name == "Tree" then
                    TweenPlayer(v.WorldPivot.Position)
                    useAllSkill()
                end
            end
        end
    ...
    for _, v in ipairs(enemies:GetChildren()) do
        if table.find(targetEnemies, v.Name) and v.Humanoid and v.Humanoid.Health > 0 then
            repeat
                ...
                PosMon = v.HumanoidRootPart.CFrame
                MonFarm = v.Name
            until not v.Parent or v.Humanoid.Health <= 0 or checkEagleEye() or not _G.FullyTyrant
        end
    end
end

function attackTyrant()
    local enemies = game:GetService("Workspace").Enemies
    for _, v in ipairs(enemies:GetChildren()) do
        if v.Name == "Tyrant of the Skies" and v.Humanoid and v.HumanoidRootPart and v.Humanoid.Health > 0 then
            ...
            until not v.Parent or v.Humanoid.Health <= 0 or not _G.Settings.Main["Auto Summon Tyrant Of The Skies"]
        end
    end
end

function autoSummonTyrant()
    task.spawn(function()
        while task.wait(0.2) do
            if not getgenv().AutoFullyTyrant then return end
            pcall(function()
    ...
    task.spawn(function()
        while getgenv().KillLevi do
            ...
            for _, v in pairs(game.Workspace.SeaBeasts:GetChildren()) do
                if v.Name:find("Leviathan") or (IsSameName(v.Name,"Leviathan [Raid Boss]") or IsSameName(v.Name,"Leviathan")) and v:FindFirstChild("HumanoidRootPart") and v.Health.Value > 0 then
                    leviathan = v
                    break
                end
            end
            if leviathan then
                ...
                repeat
                    ...
                    local seaBeastPos = rootPart.CFrame
                    local targetPos = Vector3.new(seaBeastPos.X, 30, seaBeastPos.Z)
                    local playerPos = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
                    if (playerPos - targetPos).magnitude > 150 then
                        if Farmtween then Farmtween:Stop() end
                        Farmtween = toTarget(CFrame.new(targetPos))
                    else
                        if Farmtween then Farmtween:Stop() end
                        local yPos = TypeSeabeast == 0 and 30 or 70
                        local yOffset = TypeSeabeast == 0 and 80 or 60
                        PosKillSea = Vector3.new(seaBeastPos.X, yOffset, seaBeastPos.Z)
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(seaBeastPos.X, yPos, seaBeastPos.Z)
                        if not getgenv().SeaSkill then
                            getgenv().SeaSkill = true
                            if not IsHakiActive() then AutoHaki() end
                            AimBotSkillPosition = rootPart
                            Skillaimbot = true
                        end
                    end
                until not getgenv().KillLevi  or not leviathan.Parent or not leviathan:FindFirstChild("HumanoidRootPart") or leviathan.Health.Value <= 0
                getgenv().SeaSkill = false
                Skillaimbot = false
                NoClip = false
            end
